...
class Insights {
	...
	private function redirect_to() {
		$request_uri  = ! empty( $_SERVER['REQUEST_URI'] ) ? wp_parse_url( esc_url_raw( $_SERVER['REQUEST_URI'] ), PHP_URL_PATH ) : '';
		$query_string = wp_parse_url( esc_url_raw( $_SERVER['REQUEST_URI'] ), PHP_URL_QUERY );
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				$("#wpinsights-goodbye-link-<?php echo esc_attr( $this->plugin_name ); ?>").on("click", function() {
					...
					$(".wpinsights-goodbye-form-wrapper-<?php echo esc_attr( $this->plugin_name ); ?> #wpinsights-goodbye-form").html('<?php echo wp_kses_post($html); ?>' + '<div class="wpinsights-goodbye-form-footer"><div class="wpinsights-goodbye-form-buttons"><a id="wpinsights-submit-form-<?php echo esc_attr( $this->plugin_name ); ?>" class="wpinsights-submit-btn" href="#"><?php esc_html_e( 'Submit and Deactivate', 'wpinsight' ); ?></a>&nbsp;<a class="wpsp-put-deactivate-btn" href="' + url + '"><?php esc_html_e( 'Skip & Deactivate', 'wpinsight' ); ?></a></div></div>');
